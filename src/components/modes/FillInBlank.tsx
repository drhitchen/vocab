...
export default function FillInBlank({ words, onAnswer, onSessionEnd }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10 flex flex-col gap-8">
      ...
      <div className="rounded-2xl border border-slate-700 bg-slate-900 p-8 space-y-3">
        <p className="text-slate-300 text-lg leading-relaxed">{word.definition}</p>
        <p className="text-slate-500 text-sm">
          First letter: <span className="text-indigo-400 font-bold">{word.word[0].toUpperCase()}</span>
          {word.word.includes(' ') && (
            <span className="ml-3 text-slate-600">({word.word.split(' ').length} words)</span>
          )}
        </p>
      </div>
